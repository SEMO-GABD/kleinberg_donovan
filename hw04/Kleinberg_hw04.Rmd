...
Answer for question 2: 234 rows, 11 columns. Chunk displays dimensions of the plot.

```{r}
dim(mpg)
```

Answer for question 3: drv describes whether a car is forward, rear or all wheel drive. Descriptions of variables for mpg are shown.

```{r}
?mpg
```

Answer for question 4: Chunk shows a plot of hwy vs cyl.
...
Answer for question 5: This plot doesn't give anything really useful for statistical comparison. Chunk shows a plot of class vs drv.
...
Answer to question 4: Assuming the map works with continuous variables the two combine on the plot. Chunk shows a combo of color and shape plotting cyl vs hwy.
...
Answer for question 5: Stroke controls how thick the borders of a filled shape are. Chunk describes functions of aesthetics and plots a graph with a thick stroke.
...
Answer for question 6: The graph will have color assigned on a true/false basis of the qualifying factor. Chunk prints out a table of class vs. drv with a true/false color assignment based on cyl >5.
...
Answer for question 2: The empty cells show that there are no data points within that type of wrap. For example there are no data points collected using a 4 cyl, r drive car and as such that rid is left empty. 
...
Answer for question 5: nrows and ncols sets the number of rows and columns in the plot. Other options are drop, dir, strip.position, labeller and as.table. facet_grid has facets for more than one variable which prevents manual setting of ncol or nrow. This chunk brings up the functions of facet_wrap.
